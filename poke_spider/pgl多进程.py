...
def getSeasonPokemonDetail(id_, s=latest_season): # pokemon详情
	...
	for n in ['1', '2', '5', '6']:
		...
		detail_list = r.text
		# cur.execute('insert into pgl_getseasonpokemond_%s_%s (name, value) values (%%s, %%s) where not EXISTS (select * from pgl_getseasonpokemond_%s_%s where name=%%s)'%(s, n, s, n), (id_, detail_list, id_))
